...
class _FunctionCollector(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        self._make_item(node, "function")
        self.scope_stack.append(str(node.name))
        self.generic_visit(node)
        self.scope_stack.pop()

    def visit_AsyncFunctionDef(self, node: ast.AsyncFunctionDef) -> None:
        self._make_item(node, "async_function")
        self.scope_stack.append(str(node.name))
        self.generic_visit(node)
        self.scope_stack.pop()
...
def _scan_with_fail_soft_recovery(normalized: str) -> List[FunctionItem]:
    ...
    items = sorted(
        collector.items,
        key=lambda x: (int(getattr(x, "lineno", 0) or 0), str(getattr(x, "path", "") or "")),
    )
    return items
...
def scan_functions_from_file(file_path: str) -> List[FunctionItem]:
    try:
        ...
        raise FunctionScanError(f"Dosya utf-8 ile okunamadı: {file_path}") from exc
    except OSError as exc:
        raise FunctionScanError(f"Dosya okunamadı: {file_path}") from exc
