...
def _parse_single_function_module(code: str) -> ast.Module:
    ...
    if not isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
        raise FunctionReplaceError(
            "Yeni kod yalnızca tek bir 'def' veya 'async def' içermelidir."
        )

    return module


def _get_single_function_node(
    module: ast.Module,
...
def _validate_kind_compatibility(target_item: FunctionItem, new_node: ast.AST) -> None:
    ...
    if target_is_async and not new_is_async:
        raise FunctionReplaceError(
            "Seçilen hedef async türde. Yeni kod da 'async def' olmalıdır."
        )

    if (not target_is_async) and new_is_async:
        raise FunctionReplaceError(
            "Seçilen hedef normal türde. Yeni kod 'def' olmalıdır."
        )
...
def _prepare_replacement_code(
    ...
    indented = _indent_code_block(
        dedented,
        int(getattr(target_item, "col_offset", 0) or 0),
    )
