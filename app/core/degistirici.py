...
def _find_nearby_matching_range(source_code: str, target_item: FunctionItem, window: int = 8) -> Optional[tuple[int, int]]:
    """
...
def update_function_in_file(
    ...
    *,
    ...
    try:
        ...
        path_obj.write_text(updated_code, encoding=encoding)
    except FunctionReplaceError:
        raise
    except Exception as exc:
        raise FunctionReplaceError(
            f"Güncellenmiş dosya yazılamadı: {path_obj}"
        ) from exc
