...
NOT:
- Varsayılan olarak TEST reklam kullanılmaktadır
...
- API 34 hedefi için activity, bridge ve platform kontrolleri güçlendirilmiştir
- Android dışı ortamlarda güvenli fallback davranışı uygulanır
- Bridge bulunamazsa veya activity alınamazsa servis kontrollü hata üretir
...
class ReklamServisi:
    ...
    def _android_bridge(self):
        if platform != "android":
            raise RuntimeError("Android ortamı değil.")
        ...
        try:
            ...
            if activity is None:
                raise RuntimeError("Android activity alınamadı.")

            AdMobBridge = autoclass("org.fy.bridge.AdMobBridge")
            return activity, AdMobBridge
        except Exception as exc:
            raise RuntimeError(f"AdMob bridge alınamadı: {exc}") from exc

    def _safe_ad_unit_id(self, use_real_ad: bool) -> str:
    ...
    def reklam_yukle(self, use_real_ad: bool = False) -> None:
        ...
        try:
            ...
            ad_unit_id = self._safe_ad_unit_id(use_real_ad)
            ...
            self._banner_yuklendi = False
            self._debug_add(f"HATA(yukle): {type(exc).__name__}: {exc}")
            raise
    ...
    def reklam_goster(self) -> None:
        ...
        try:
            ...
            self._debug_add(f"HATA(goster): {type(exc).__name__}: {exc}")
            raise
    ...
    def reklam_test_et(self, use_real_ad: bool = False) -> None:
        ...
        try:
            ...
            try:
                ...
                activity = cast("android.app.Activity", PythonActivity.mActivity)
                if activity is None:
                    raise RuntimeError("mActivity boş geldi.")
                self._debug_add("Adım 3: mActivity alındı.")

                autoclass("org.fy.bridge.AdMobBridge")
                self._debug_add("Adım 4: AdMobBridge bulundu.")
            except Exception as exc:
                raise RuntimeError(f"Bridge ön kontrolü başarısız: {exc}") from exc
