...
ROL:
- Root dosya akışı modülüne merkezi erişim sağlar
- RootDosyaAkisiMixin sınıfını lazy import ile yükler
...
class RootDosyaAkisiYonetici:
    ...
    def _modul_yukle(self):
        ...
        try:
            ...
            print(f"[ROOT_DOSYA_AKISI] Modül yüklenemedi: {self.MODUL_YOLU}")
            print(exc)
            self._modul = None
            return None
    ...
    def _mixin_sinifini_yukle(self):
        """
        RootDosyaAkisiMixin sınıfını yükler.
        ...
        try:
            ...
            print(f"[ROOT_DOSYA_AKISI] Sınıf alınamadı: {self.SINIF_ADI}")
            print(exc)
            self._mixin_sinifi = None
            return None

        if sinif is None:
            print(f"[ROOT_DOSYA_AKISI] Sınıf bulunamadı: {self.SINIF_ADI}")
            self._mixin_sinifi = None
            return None
