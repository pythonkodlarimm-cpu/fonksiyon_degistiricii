# -*- coding: utf-8 -*-
"""
DOSYA: app/ui/root_paketi/kaydirma/scroll.py

ROL:
- Root katmanında güvenli scroll davranışını yönetmek
- Fonksiyon listesi ve editör alanına programatik kaydırma sağlamak
- Editör odak akışını güvenli şekilde yürütmek

MİMARİ:
- UI bileşenlerini doğrudan kontrol eder
- Hata durumlarında akışı bozmaz
- Scroll ve focus işlemlerini Clock ile gecikmeli tetikler
- Root paketinin alt kaydırma modülüdür
...
class RootScrollMixin:
    ...
    def _scroll_to_function_list(self) -> None:
        try:
            ...
            def _go(*_args):
                self._safe_scroll_to_widget(
                    self.function_list,
                    extra_top_padding=dp(120),
                )
