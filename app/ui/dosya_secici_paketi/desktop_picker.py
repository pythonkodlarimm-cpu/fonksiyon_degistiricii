...
class DesktopPicker:
    ...
    def open_popup(self):
        ...
        mevcut_klasor = varsayilan_baslangic_klasoru()
        ...
        scroll = ScrollView(do_scroll_x=False, do_scroll_y=True, bar_width=dp(8), scroll_type=["bars", "content"])
        liste = GridLayout(cols=1, spacing=dp(6), padding=(0, dp(4)), size_hint_y=None)
        liste.bind(minimum_height=liste.setter("height"))
        scroll.add_widget(liste)
        ana.add_widget(scroll)

        popup = Popup(title="", content=ana, size_hint=(0.96, 0.96), auto_dismiss=False, separator_height=0)
        ...
        def listeyi_yenile(*_args):
            ...
            try:
                ...
                for oge in sorted(mevcut_klasor.iterdir(), key=lambda p: (not p.is_dir(), p.name.lower())):
                    try:
                        if oge.is_dir():
                ...
                for klasor in klasorler:
                    self._satir(liste, "[KLASOR]  " + klasor.name, lambda _btn, p=klasor: klasore_gir(p), CARD_BG)

                for dosya in py_dosyalar:
                    self._satir(liste, "[PY]  " + dosya.name, lambda _btn, p=dosya: dosya_sec(p), CARD_BG_SOFT)

                if not py_dosyalar:
                    for dosya in diger:
                        self._satir(liste, "[DOSYA]  " + dosya.name, lambda _btn, p=dosya: dosya_sec(p), CARD_BG_DARK)
