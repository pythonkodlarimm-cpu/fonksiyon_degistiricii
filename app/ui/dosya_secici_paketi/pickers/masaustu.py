# -*- coding: utf-8 -*-
"""
DOSYA: app/ui/dosya_secici_paketi/pickers/masaustu.py
...
- Doğrudan Android özel çağrısı içermez
- Görünüm korunmuş, güvenlik ve dayanıklılık artırılmıştır

API UYUMLULUK:
- Masaüstü ve path tabanlı ortamlarda güvenli çalışır
- Android dışı fallback picker olarak kullanılabilir
- Yeni klasör mimarisi ile uyumludur
...
class _SatirKart(BoxLayout):
    ...
    def _guncelle(self, *_args):
        self._bg_rect.pos = self.pos
        self._bg_rect.size = self.size


class MasaustuSecici:
    ...
    def _debug(self, message: str) -> None:
        try:
            self.owner._debug(f"[MASAUSTU_SECICI] {message}")
        except Exception:
            try:
                print("[MASAUSTU_SECICI]", str(message))
            except Exception:
                pass
    ...
    def _initial_folder(self) -> Path:
        try:
            path_obj = self._safe_path(varsayilan_baslangic_klasoru())
    ...
    def secici_ac(self):
        ...
        def listeyi_yenile(*_args):
            ...
            try:
                ...
                if not klasorler and not py_dosyalar and not diger_dosyalar:
                    self._satir(
                        liste,
