...
class KodEditoru(CodeInput):
    ...
    line_number_label = ObjectProperty(None, allownone=True)
...
class EditorPaneli(BoxLayout):
    ...
    def _looks_like_full_function(self, text: str) -> bool:
        first_line = self._first_meaningful_line(text)
        if not first_line:
            return False
        return first_line.startswith("def ") or first_line.startswith("async def ")
    ...
    def _show_fold_not_supported(self, *_args):
        self._set_error(
            "Gerçek kod katlama bu editörde yerleşik değil. Custom widget gerekir.",
            0,
        )
