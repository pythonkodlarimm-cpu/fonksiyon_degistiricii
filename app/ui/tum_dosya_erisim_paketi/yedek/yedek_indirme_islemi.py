...
- Popup katmanına ortak services instance'ını aşağı aktarmak
- Görünen hata metinlerini dil servisi ile uyumlu üretmek
...
- Services verilirse popup ve hata metinleri aktif dil üzerinden çözülür
- Services verilmezse güvenli fallback ile çalışır
- Mevcut fonksiyon imzası korunur
    ...
    try:
        ...
        if not yedek.exists():
            raise FileNotFoundError(
                f"{_m(services, 'backup_not_found', 'Yedek bulunamadı')}: {yedek}"
            )

        if not yedek.is_file():
            raise FileNotFoundError(
                f"{_m(services, 'invalid_backup_file', 'Geçerli bir yedek dosyası değil')}: {yedek}"
            )
        ...
        if not hedef.exists():
            raise FileNotFoundError(
                f"{_m(services, 'download_target_not_created', 'İndirilen hedef dosya oluşmadı')}: {hedef}"
            )
        ...
        if hedef_ad:
            ek_metin = (
                f"\n{_m(services, 'target_folder', 'Hedef klasör')}: {hedef_ad}"
            )

        popups.show_simple_popup(
            title_text=_m(services, "download_error_title", "İndirme Hatası"),
            body_text=(
