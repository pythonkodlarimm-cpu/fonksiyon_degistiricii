# -*- coding: utf-8 -*-
"""
DOSYA: app/ui/tum_dosya_erisim_paketi/ortak/bilesenler.py

ROL:
- Tüm dosya erişim paketi içinde ortak kullanılan UI bileşenlerini sağlamak
- Tıklanabilir ikon, animasyonlu ayraç ve ikon glow yardımcı akışını sunmak
- Panel ve popup katmanlarının tekrar kullanılabilir görsel parçalarını merkezileştirmek

MİMARİ:
- Saf UI bileşenleri içerir
- İş mantığı taşımaz
- Üst katman bu modüle doğrudan değil, ortak/yoneticisi.py üzerinden erişmelidir
...
class TiklanabilirIcon(ButtonBehavior, Image):
    """Tıklanabilir basit ikon bileşeni."""
    pass


class AnimatedSeparator(BoxLayout):
    """Renk geçişli animasyonlu yatay ayraç."""
...
def start_icon_glow(widget, size_small_dp=36, size_big_dp=40, duration=0.60):
    """
    Verilen widget üzerinde tekrar eden yumuşak glow/pulse animasyonu başlatır.

    Dönüş:
    - başarılıysa Animation nesnesi
    - başarısızsa None
    """
    try:
        anim = (
            Animation(
                opacity=0.72,
                size=(dp(size_big_dp), dp(size_big_dp)),
                duration=duration,
            )
            + Animation(
                opacity=1.0,
                size=(dp(size_small_dp), dp(size_small_dp)),
                duration=duration,
            )
        )
        anim.repeat = True
