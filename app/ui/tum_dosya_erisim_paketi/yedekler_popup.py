...
from app.ui.tum_dosya_erisim_paketi.onay_popup import show_confirm_popup
from app.ui.tum_dosya_erisim_paketi.silme_durum_popup import SilmeDurumPopup


def open_backups_popup(debug=None):
    from app.services.yedek_listeleme_servisi import yedekleri_listele
    from app.services.yedek_silme_servisi import tum_yedekleri_sil, yedegi_sil
    ...
    tumunu_sil_btn = TiklanabilirIcon(
    ...
    def _yedekleri_getir():
        ...
        if tip == "bugun":
            return yedekleri_listele(bugun=True)
        if tip == "dun":
            return yedekleri_listele(dun=True)
        if tip == "son7":
            return yedekleri_listele(son_gun=7)
        if tip == "tarih" and tarih:
            return yedekleri_listele(tarih=tarih)

        return yedekleri_listele()

    def _ilk_filtreyi_belirle():
        try:
            bugun_listesi = yedekleri_listele(bugun=True)
            if bugun_listesi:
        ...
        try:
            dun_listesi = yedekleri_listele(dun=True)
            if dun_listesi:
        ...
        try:
            son7_listesi = yedekleri_listele(son_gun=7)
            if son7_listesi:
    ...
    def refresh_list():
        ...
        for yedek in yedekler:
            def on_view(y):
                from app.ui.tum_dosya_erisim_paketi.yedek_goruntuleme_popup import open_backup_view_popup
                open_backup_view_popup(y)

            def on_download(y):
                try:
                    from app.ui.tum_dosya_erisim_paketi.yedek_indirme_islemi import backup_download_action
                    backup_download_action(debug or (lambda *_: None), y)
                except Exception as exc:
                    show_simple_popup(
            ...
            try:
                from app.ui.tum_dosya_erisim_paketi.yedek_satiri import build_backup_row
                liste.add_widget(
                    build_backup_row(yedek, on_view, on_download, on_delete)
                )
            except Exception as exc:
    ...
    def _toplu_sil_onayli():
        ...
        def _silme_adimi(_dt):
            try:
                ...
                silinen_sayi = tum_yedekleri_sil()
