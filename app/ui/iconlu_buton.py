...
- İkon yoksa placeholder ile hizalama korunur
- Basış animasyonu korunur
- Disabled görünümü korunur
"""
...
class IconluButon(ButtonBehavior, BoxLayout):
    ...
    def _create_placeholder(self) -> Widget:
        return Widget(
            size_hint=(None, None),
            size=(self._icon_px, self._icon_px),
        )

    def _build_icon_area(self) -> None:
        source = icon_path(self._icon_name)

        if source:
            self.icon = self._create_icon_widget(source)
            self.add_widget(self.icon)
            self._icon_placeholder = None
        else:
            self.icon = None
            self._icon_placeholder = self._create_placeholder()
    ...
    def set_icon(self, icon_name: str) -> None:
        ...
        if self.icon is None:
            ...
            try:
                ...
                self.icon.color = self._normal_fg if not self.disabled else self._disabled_fg
                self.icon.reload()
            except Exception:
                pass
