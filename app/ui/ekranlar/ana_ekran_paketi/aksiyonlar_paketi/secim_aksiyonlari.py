...
class AnaEkranSecimAksiyonMixin:
    ...
    def _secili_kaynagi_uygula(self, kaynak: str) -> None:
        ...
        Not:
        - Bu metod UI'ya dokunduğu için ana thread üzerinde çalıştırılmalıdır
        - Dışarıdan background callback ile çağrılacaksa
          ui_thread_uzerinde_calistir(...) üzerinden çağrılmalıdır
        """
    ...
    def _fonksiyon_secildi(self, item: FunctionItem | None) -> None:
        ...
        Not:
        - UI güncellemesi içerdiği için ana thread güvenliği uygulanır
        """
        ...
        try:
            ...
            Logger.exception(
                f"AnaEkranSecimAksiyon: _fonksiyon_secildi ui thread hatasi: {exc}"
            )
    ...
    def _listeyi_yukle(self, items: list[FunctionItem] | None) -> None:
        ...
        Not:
        - Tarama sonrası callback farklı thread'den gelebileceği için
          tüm UI etkileri ana thread'e alınır
        """
        liste = list(items or [])
        ...
        try:
            ...
            Logger.exception(
                f"AnaEkranSecimAksiyon: _listeyi_yukle ui thread hatasi: {exc}"
            )
            if self._bilgi is not None:
                self._bilgi.mesaj(f"{self._t('error_title')}: {exc}", hata=True)
        ...
        def _secildi(kaynak: str) -> None:
            ...
            def _ui() -> None:
                ...
                try:
                    ...
                    Logger.exception(
                        f"AnaEkranSecimAksiyon: _secildi ui akisi hata: {exc}"
                    )
                    if self._bilgi is not None:
                        self._bilgi.mesaj(
            ...
            try:
                ...
                Logger.exception(
                    f"AnaEkranSecimAksiyon: _secildi schedule hatasi: {exc}"
                )
                if self._bilgi is not None:
                    self._bilgi.mesaj(f"{self._t('error_title')}: {exc}", hata=True)
