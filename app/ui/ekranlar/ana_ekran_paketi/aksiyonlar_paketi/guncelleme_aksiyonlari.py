...
- Popup / reklam / callback kaynaklı tüm UI etkilerini ortak ui_thread katmanı ile
  ana thread üzerinde yürütür
...
class AnaEkranGuncellemeAksiyonMixin:
    ...
    def _guncelleme_basarisi_goster(self) -> None:
        """
        ...
        try:
            ...
            Logger.exception(
                "AnaEkranGuncellemeAksiyon: guncelleme basari mesaji "
                f"gosterilemedi: {exc}"
            )
    ...
    def _geri_yukleme_basarisi_goster(self) -> None:
        """
        ...
        try:
            ...
            Logger.exception(
                "AnaEkranGuncellemeAksiyon: geri yukleme basari mesaji "
                f"gosterilemedi: {exc}"
            )
            if self._bilgi is not None:
                self._bilgi.mesaj(
    ...
    def _tarama_hatasi_mesaji_goster(self, mesaj: str) -> None:
        """
        Tarama hatasını ortak feedback pipeline ile gösterir.
        """
        try:
    ...
    def _guncelle_sonrasi_akisi(self) -> None:
        ...
        def _ui() -> None:
            try:
                self._tara()
                ui_thread_uzerinde_calistir(self._guncelleme_basarisi_goster)
            except Exception as exc:
                Logger.exception(
                ...
                ui_thread_uzerinde_calistir(
                    self._tarama_hatasi_mesaji_goster,
                    hata_mesaji,
                )
    ...
    def _geri_yukle_sonrasi_akisi(self) -> None:
        ...
        def _ui() -> None:
            try:
                self._tara()
                ui_thread_uzerinde_calistir(self._geri_yukleme_basarisi_goster)
            except Exception as exc:
                Logger.exception(
